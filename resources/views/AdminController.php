...

{
    ...
    public function userList()
    {
        ...
        $data["user_list"] = User::where("user_type_id", "3")->paginate(2);
    ...
    public function userLists()
    {
        ...
        $data["user_listing"] = User::where("user_type_id", "2")->paginate(5);
    ...
    public function xrayList()
    {
        ...
        $data["xray_type"] = XrayTypes::where("status", 1)->paginate(5);
    ...
    public function deletegallary($id)
   {
         ...
         public function assetsType()
    {
        ...
        $data ["asset_list"]= Assets::paginate(7);
        
        return view('backEnd/assets_list',$data);
    }
    public function createassetsType(Request $request)
    {
        $data["asset_list"] = Assets::all();
        return view("backEnd/createasset", $data);
    }
    ...
    {
        $data["title"] = "Asset Type";
        $data["asset_list"] = Assets::find($id);
        $asset_list = Assets::find($id);
        return view("backEnd/editassets", $data);
    } 
     public function updateupdateassets(Request $request, $id)
    {
        $product_list = Assets::find($id);
        $product_list->assets_name = $request->input("assets_name");
        $product_list->status = $request->input("status");
        $product_list->update();
        Session::flash('update',"data has been updated succesfully");
    ...
    public function deletassets($id)
   {
   ...
   public function assetsbrandList()
    {
        $data["title"] = "Asset Brand";
        $data["brand_list"] = AssetBrand::all();
        return view("backEnd/brandlist", $data);
    }
     public function createassetsBrand(Request $request)
    {
        $data["brand_list"] = AssetBrand::all();
        return view("backEnd/createbrand", $data);
    }
      public function savebrand(Request $request)
    {
        ...
        return redirect("assetbrand");
    }
      public function editbrands($id)
    {
        $data["title"] = "Asset Type";
        $data["brand_list"] = AssetBrand::find($id);
        $brand_list = AssetBrand::find($id);
        return view("backEnd/editbrands", $data);
    } 
     public function updatebrands(Request $request, $id)
    {
        $product_list = AssetBrand::find($id);
        $product_list->assets_type_id = $request->input("assets_type_id");
        $product_list->brand_name = $request->input("brand_name");
        $product_list->status = $request->input("status");
        $product_list->update();
        Session::flash('update',"data has been updated succesfully");
    ...
    public function deletbrands($id)
   {

       AssetBrand::destroy($id);
       Session::flash('error', "gallary List has been deleted succesfully !!");

        return redirect("assetbrand");
   }
   public function createModels(){
      $data['title'] = 'Create Models';
      $data["asset_list"] = Assets::all();
      $data["brand_list"] = AssetBrand::all();

     return view('backEnd/createModels',$data);
    }
    ...
    public function savemodels(Request $request)
    {
        ...
        if($input['assets_type_id'] =='Sim Card'){
                DB::table('model_details')->insert([
                    'assets_type_id' =>$input['assets_type_id'],    
                    'sim' => $input['sim'],
                    'sim_number' =>  $input['sim_number'],
                    'imsi_number' =>  $input['imsi_number'],
                    'contact_number' =>  $input['contact_number'],
                    'created_at' => $now->format('Y-m-d H:i:s'),
                    'updated_at' => $now->format('Y-m-d H:i:s')
                ]);       
            }else{
            AssetDetails::create($input);
        }
        Session::flash('success',"Data has been Added succesfully");

         return redirect("createModels");
    }
